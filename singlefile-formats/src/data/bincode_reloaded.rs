#![cfg_attr(docsrs, doc(cfg(feature = "bincode-reloaded")))]
#![cfg(feature = "bincode-reloaded")]

//! Defines a [`FileFormat`] using the Bincode binary data format.

pub extern crate bincode_reloaded as original;

use bincode_reloaded::{Decode, Encode};
use bincode_reloaded::config::{Configuration, Config, BigEndian, LittleEndian, Varint, Fixint, Limit, NoLimit};
use bincode_reloaded::error::{DecodeError, EncodeError};
...
where T: Decode<()> + Encode, Configuration<E, I, L>: Config {
  ...
  fn from_reader<R: Read>(&self, mut reader: R) -> Result<T, Self::FormatError> {
    Ok(bincode_reloaded::decode_from_std_read(&mut reader, self.configuration)?)
  }

  #[inline]
  fn from_buffer(&self, buf: &[u8]) -> Result<T, Self::FormatError> {
    Ok(bincode_reloaded::decode_from_slice(buf, self.configuration)?.0)
  }

  #[inline]
  fn to_writer<W: Write>(&self, mut writer: W, value: &T) -> Result<(), Self::FormatError> {
    bincode_reloaded::encode_into_std_write(value, &mut writer, self.configuration)?;
    Ok(())
  }

  #[inline]
  fn to_buffer(&self, value: &T) -> Result<Vec<u8>, Self::FormatError> {
    Ok(bincode_reloaded::encode_to_vec(value, self.configuration)?)
  }
}
...
where T: DeserializeOwned + Serialize, Configuration<E, I, L>: Config {
  ...
  #[inline]
  fn from_reader<R: Read>(&self, mut reader: R) -> Result<T, Self::FormatError> {
    Ok(bincode_reloaded::serde::decode_from_std_read(&mut reader, self.configuration)?)
  }

  #[inline]
  fn from_buffer(&self, buf: &[u8]) -> Result<T, Self::FormatError> {
    Ok(bincode_reloaded::serde::decode_from_slice(buf, self.configuration)?.0)
  }

  #[inline]
  fn to_writer<W: Write>(&self, mut writer: W, value: &T) -> Result<(), Self::FormatError> {
    bincode_reloaded::serde::encode_into_std_write(value, &mut writer, self.configuration)?;
    Ok(())
  }

  #[inline]
  fn to_buffer(&self, value: &T) -> Result<Vec<u8>, Self::FormatError> {
    Ok(bincode_reloaded::serde::encode_to_vec(value, self.configuration)?)
  }
}
